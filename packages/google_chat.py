...
import pytz
import requests
from util import DagError, helper
...
class GoogleChat:
    ...
    @staticmethod
    @helper.logit
    def df_to_sections(df: pd.DataFrame, index_col: str = None) -> Sections:
        if df.empty:
    ...
    @staticmethod
    @helper.logit
    def buttons_to_section(buttons: Buttons, header: str = None) -> Section:
        if not isinstance(buttons, Buttons):
            raise DagError(f"Func buttons_to_section: Not a Buttons obj {buttons=}")
        return Section(
            {
                "header": header or "",
    ...
    @staticmethod
    def hyperlink_to_button(url: str, text: str = None, icon: str = None) -> Button:
        return Button(
            {
                "text": text,
                "icon": {
    ...
    def exception_to_sections(exception: Exception, instance_url: str) -> Sections:
        ...
        if isinstance(exception_message, pd.DataFrame):
        ...
        elif exception_message.__class__.__name__ == "Sections":
            sections = exception_message
            ...
            try:
                ...
                logger.warning(
                    f"Exception raised is a list but not structured for Section: {e}"
                )
        ...
        if "sections" not in locals():
            exception_extract = helper.condense_text(str(exception_message), limit=3500)
            sections = [GoogleChat.text_to_section(exception_extract, header=header)]

        button: Button = GoogleChat._instance_button(instance_url)
        sections.append(button)
        return sections

    @staticmethod
    def _instance_button(instnace_url: str) -> Button:
    ...
    def send_gchat(
        ...
        if not tag_user_ids:
            ...
            raise DagError(f"Invalid {tag_user_ids=}")
    ...
    def send_card(
        self,
        sections: Union[Sections, list, Section],
        webhook: str,
        title: str,
        subtitle: str,
        footer: Union[Section, str] = None,
        tag_user_ids: list = None,
        card_id: str = None,
    ):
        if sections.__class__.__name__ == "Sections":
            pass
        elif isinstance(sections, list):
            sections = Sections(*sections)
        elif sections.__class__.__name__ == "Section":
            sections = Sections(sections)
        else:
            raise DagError(
...
def main():
    img_section = gchat.hyperlink_to_image_section(
        url="https://www.google.com/images/branding/googlelogo/2x/googlelogo_color_92x30dp.png",
        alt_text="Google",
        header="Google Header",
    )
    sections = Sections(
        gchat.text_to_section("This is a test message", header="Test Header"),
        img_section,
    )
    gchat.send_card(
        sections=sections,
